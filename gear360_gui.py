...
class StitcherGUI:
    ...
    def create_single_tab(self):
        ...
        ttk.Label(preset_frame, text="Load Preset:").grid(row=0, column=0, sticky=tk.W, padx=5)
        ...
        ttk.Label(settings_frame, text="degrees").grid(row=1, column=2, sticky=tk.W)
    ...
    def create_batch_tab(self):
        ...
        # Progress
        progress_frame = ttk.LabelFrame(tab, text="Batch Progress", padding="10")
        ...
        self.batch_progress_bar.grid(row=0, column=0, sticky=(tk.W, tk.E), pady=5)
    ...
    def create_calibration_tab(self):
        ...
        row += 1
        distortion_frame = ttk.LabelFrame(tab, text="Distortion Coefficients", padding="10")
        distortion_frame.grid(row=row, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=20)
        ...
        info_frame.grid(row=row, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=10)
...
"""
        ...
        def save():
            ...
            if name:
                try:
                    ...
                    self.preset_manager.add_preset(name, width, height, rotation)
                    self.preset_combo['values'] = list(self.preset_manager.presets.keys())
    ...
    def preview_worker(self):
        ...
        try:
            ...
            self.log_queue.put(('error', f"Error generating preview: {str(e)}"))
    ...
    def save_calibration_manual(self):
        ...
        if filename:
            try:
                ...
                messagebox.showerror("Error", "Invalid distortion coefficient values")
    ...
    def load_calibration_manual(self):
        ...
        if filename:
            try:
                ...
                messagebox.showerror("Error", f"Could not load calibration: {str(e)}")
    ...
    def batch_worker(self):
        ...
        try:
            ...
            for i, input_file in enumerate(self.batch_files):
                ...
                self.log_queue.put(('status', f"Processing {i+1}/{total_videos}: {input_path.name}"))
                self.log_queue.put(('log', f"\n{'='*60}"))
            ...
            if not self.cancel_requested:
                ...
                self.log_queue.put(('done', 'Batch processing cancelled'))
                
        except Exception as e:
            self.log_queue.put(('error', f"Error during batch processing: {str(e)}"))
